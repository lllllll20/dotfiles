...
#+begin_src emacs-lisp
  ...
  (keymap-set my-dired-goto-map "<escape>" 'keyboard-quit)
...
#+begin_src emacs-lisp
  ...
  (defun my-test-keys-insert-mode-escape ()
    (interactive)
    (cond
     ;; 1. Deactivate any active selection
     ((region-active-p)
      (deactivate-mark))

     ;; 2. Exit the minibuffer if it's open
     ((active-minibuffer-window)
      (abort-recursive-edit))

     ;; 3. FIX: Clear prefix command state (like the 'g' goto menu)
     ;; This prevents the "prefix command" error when hitting escape
     (overriding-terminal-local-map
      (setq overriding-terminal-local-map nil)
      (message "Quit prefix command"))

     ;; 4. Handle Dired/Ranger/File-Manager states
     ((derived-mode-p 'ranger-mode 'dired-mode 'my-file-manager-mode)
      (if (derived-mode-p 'ranger-mode)
          (ranger-close)
        (my-test-keys-command-mode-activate)))

     ;; 5. Default: Switch to Command Mode
     (t
      (my-test-keys-command-mode-activate))))
  ...
  (defun my-test-keys-insert-mode-escape ()
    (interactive)
    (cond
     ;; 1. Deactivate any active text selection
     ((region-active-p) (deactivate-mark))
     
     ;; 2. Exit the minibuffer if it's open
     ((active-minibuffer-window) (abort-recursive-edit))

     ;; 3. FIX: Clear the "waiting for key" state of the 'g' goto menu
     ;; This prevents the prefix command error you mentioned
     (overriding-terminal-local-map
      (setq overriding-terminal-local-map nil)
      (message "Quit prefix command"))

     ;; 4. If in Ranger, Dired, or your unified File Manager mode, call ranger-close
     ((derived-mode-p 'ranger-mode 'dired-mode 'my-file-manager-mode)
      (ranger-close))

     ;; 5. Default: Switch back to Command Mode
     (t (my-test-keys-command-mode-activate))))
